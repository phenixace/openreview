...
test("ships game-specific metadata and social artwork", async () => {
  ...
  assert.match(page, /你给大佬留下了坏印象/);
  assert.match(page, /圈内好感度/);
  assert.match(page, /大佬好感度/);
  assert.match(page, /bossFavor/);
  assert.match(page, /circle_favor: favor/);
  assert.match(page, /boss_favor: bossFavor/);
  assert.doesNotMatch(page, /大佬凝视/);
  assert.match(page, /trainingSuccessChance/);
  assert.match(page, /休整恢复精力/);
  assert.match(page, /去圈内 Social/);
  assert.match(page, /青年学者交流群 48/);
  assert.match(page, /进修失败/);
  assert.match(page, /arxivExposureRisk/);
  assert.match(page, /rawArxivPriorEffect = origin\.arxiv \+ arxivFavorAdjustment/);
  assert.match(page, /Math\.min\(-2, rawArxivPriorEffect\)/);
  assert.match(page, /rawArxivExposureRisk = origin\.arxivExposure - arxivFavorAdjustment/);
  assert.match(page, /clamp\(rawArxivExposureRisk, 5, 40\)/);
  assert.match(page, /暴露风险/);
  assert.match(page, /clamp\(value \+ result\.bossFavorDelta, -100, 100\)/);
  assert.match(page, /reportAutoResearch/);
  assert.match(page, /主动举报 AutoResearch/);
  assert.match(page, /reportSuccessChance/);
  assert.match(page, /未发现明确 AutoResearch 痕迹/);
  assert.doesNotMatch(page, /模板痕迹不明/);
  ...
  assert.match(page, /boss-retaliation/);
